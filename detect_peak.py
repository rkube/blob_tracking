...
def detect_peak_3d(signal, trigger_box, minmax, lag, rel_idx=False):

    """
    Given a timeseries of 2d surfaces, identify peaks within a specified
    intervall that occur in a trigger subarray in the timeseries. Sort the
    peaks by amplitude and succesively eliminate all peaks occuring within
    ...
    Input:
        ...
        rel_idx          : If true, return tidx, ridx, zidx as absolute indices to
    ...
    tseries = signal[:, trigger_box[2]:trigger_box[3], trigger_box[0]:trigger_box[1]]

    #print 'Detecting peaks on tseries shape:', tseries.shape
    #print 'tseries.min = %f, tseries.max = %f, peaks in %f,%f' % (tseries.min(),
    #        tseries.max(), minmax[0], minmax[1])

    # Identify position of the peaks in the timeseries falling in the
    # specified range
    # We ignore the following peaks:
    # 1.) Peaks with lower amplitude that fall in the deadtime intervall after
    #     a peak was detected
    ...
    # Create a structured array to sort the tuples
    # (Amplitude(t_i, z_i, R_i), t_i, z_i, R_i) by amplitude.
    pl = [(tseries[peak_idx[0], peak_idx[1], peak_idx[2]],
...
#        print p
    ...
    # We now have the sorted amplitudes of all detected peaks in the trigger
    # box.
    # Iterate over peaks sorted by magnitude, blank out those within the
    # deadtime window of a previous peak

    for idx, peak in enumerate(peak_list):
        # Peak was blanked out, go to next one
        if (peak['value'] == -1.0):
            continue
        # Mark all indices within the specified lag to be blanked out and
        # ignore the current index
        blank_indices = np.abs(peak_list['tidx'] - peak['tidx']) < lag
        blank_indices[idx] = False

        # Ignore the peak with index zero, as this is the current peak. Also,
        # add the relative array index idx to the peak indices that need to be
        # blanked out
        peak_list['value'][blank_indices] = -1.0
        # if ( rel_idx == False ):
        if not rel_idx:
            peak['ridx'] = peak['ridx'] + trigger_box[0]
            peak['zidx'] = peak['zidx'] + trigger_box[2]

# Debug, plot what we have found
#    for peak in peak_list:
#        if (peak['value'] > 0):
#            plt.figure()
#            title_str = 'tidx=%d ridx=%d zidx=%d' %\
#                (peak['tidx'], peak['ridx'], peak['zidx'])
#            plt.title(title_str)
#            plt.contourf(tseries[peak['tidx'], :, :])
#            plt.plot(peak['ridx'] - trigger_box[0], 
#                     peak['zidx'] - trigger_box[2], 'ko')
#            plt.colorbar()

#    print '%d peaks, sorted after amplitude:' % (np.shape(peak_list)[0])
#    for p in peak_list[peak_list['value'] > 0.0]:
#        print p
...
def detect_peak_2d(signal, px, minmax, t0, lag, rel_idx=True):
    ...
    Input:
        ...
        rel_idx : If true, return indices of events relative to t0.
                  Return absolute indices otherwise

    Output
        avg_surf    : Averaged surfaces
    """

    print 'Detecting peaks within [%f:%f] * rms at px (%d,%d)' % (minmax[0],
                                                                  minmax[1],
                                                                  px[0],
                                                                  px[1])
    ...
    # tseries[:] = (tseries_original -
    #               tseries_original.mean()) / tseries_original.std()

    # Identify position of the peaks in the timeseries falling in the
    # specified range
    # We ignore the following peaks:
    # 1.) Early and late peaks in the time series where the specified window
    #     cannot be fitted around
    # 2.) Peaks that fall in the averaging window of another peak

    # Return value from np.argwhere are indices from tseries[lag:-lag].
    # To convert these indices to indices for tseries, we have to add the
    # offset lag
    peak_indices = np.squeeze(np.argwhere((tseries[lag:-lag] > minmax[0]) &
                                          (tseries[lag:-lag] < minmax[1])))
    peak_indices += lag
    # Sort the peaks by amplitude, largest first: traverse the return from
    # argsort from back to front
    peak_indices = peak_indices[np.argsort(tseries[peak_indices])[::-1]]

    # Iterate over peaks sorted by magnitude, blank out those within the
    # cond. avg. windows of a previous peak
