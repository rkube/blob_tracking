...
"""
Estimate radial blob velocity by correlation analysis.
For a list of poloidal positions the intensity signal of GPI phantom data is correlated
at a sequence of radial pixels. The maximum of the correlation signal estimates the radial
velocity.
"""
...
jy_px = np.array([ 10, 20, 30, 40, 50, 60 ])

num_blobs = 100
blob_vel = np.zeros([np.size(jy_px), num_blobs])


for jy_idx, jy in enumerate(jy_px):

    ts_pixel1 = frames[frame0 : frame0 + nframes, jy, 51]
    ts_pixel2 = frames[frame0 : frame0 + nframes, jy, 53]
    ts_pixel3 = frames[frame0 : frame0 + nframes, jy, 55]
    ts_pixel4 = frames[frame0 : frame0 + nframes, jy, 57]
    ts_pixel5 = frames[frame0 : frame0 + nframes, jy, 59]
...
#    plt.plot( np.arange( frame0 + nframes, np.size(ts_pixel1)), ts_pixel5[frame0 + nframes:] + 9.0, 'k' )
    ...
    for idx, max_idx in enumerate(ts1_sortidx):
        ...
        blob_vel[jy_idx,idx] = np.mean( np.array([v_c12, v_c13, v_c14, v_c15]) )
    
#blob_vel = blob_vel[blob_vel != 0]
print np.shape(blob_vel)
plt.figure()
for idx in np.arange(np.shape(blob_vel)[0]):
    plt.plot(blob_vel[idx,:], '.')

plt.xlabel('Blob event no.')
plt.ylabel('Radial velocity m/s')

print '================================================================================='
print 'mean over all blobs: %f' % blob_vel[blob_vel != 0].mean()

np.savez('%d/%d_blobvels_correlation.npz' % ( shotnr, shotnr ), blob_vel = blob_vel, jy_px = jy_px)
